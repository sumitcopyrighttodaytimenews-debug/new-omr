...
old_loop = """            val totalPages = Math.ceil(shuffledQuestions.size / 50.0).toInt().coerceAtLeast(1)
            var pageIndex = 1
            ...
            for (p in 0 until totalPages) {
                ...
                // Questions
                val startQIndex = p * 50
                val endQIndex = Math.min(startQIndex + 50, shuffledQuestions.size)

                for (i in startQIndex until endQIndex) {
                    val qIndexOnPage = i - startQIndex
                    val isCol2 = qIndexOnPage >= 25
                    val colX = if (isCol2) pageW / 2f + 20f else 40f
                    val qY = 245f + (qIndexOnPage % 25) * 32f
...
if old_loop in content:
    ...
    print("Old loop not found. Looking closely at strings:")
    print("Found totalPages = " + str("totalPages = Math.ceil(shuffledQuestions.size / 50.0).toInt().coerceAtLeast(1)" in content))
