...
old_func = """    private fun findCorner(bitmap: Bitmap, isLeft: Boolean, isTop: Boolean): Pair<Float, Float> {
...
new_func = """    private fun findCorner(bitmap: Bitmap, isLeft: Boolean, isTop: Boolean): Pair<Float, Float> {
        ...
        for (x in startX until endX - windowSize step step) {
            for (y in startY until endY - windowSize step step) {
                ...
                if (total > 0 && darkCount.toFloat() / total > 0.35f) {
                    val cx = x + windowSize / 2f
                    val cy = y + windowSize / 2f
                    ...
                    // Penalize extreme edges to avoid black borders/shadows from scanning
            ...
            for (wx in (bestX - windowSize).toInt().coerceAtLeast(0) until (bestX + windowSize).toInt().coerceAtMost(width)) {
                for (wy in (bestY - windowSize).toInt().coerceAtLeast(0) until (bestY + windowSize).toInt().coerceAtMost(height)) {
                    ...
                    if (gray < threshold + 15) {
                        val distCornerX = if (isLeft) wx.toFloat() else (width - wx).toFloat()
                        val distCornerY = if (isTop) wy.toFloat() else (height - wy).toFloat()
                        val penaltyX = if (distCornerX < width * 0.015f) 500f else 0f
                        val penaltyY = if (distCornerY < height * 0.015f) 500f else 0f
                        
                        val dist = distCornerX + distCornerY + penaltyX + penaltyY
                        if (dist < extScore) {
                            extScore = dist
                            extX = wx.toFloat()
                            extY = wy.toFloat()
